...
@auto_register_config
class TruncatedVocabCausalLMPool(CausalLMPool):
    """A CausalLMPool that truncates expert vocab to match the base vocab size.

    Use this when one or more experts have a larger vocab_size than the base model
    (e.g. MergeBench/Llama-3.2-3B_math has vocab 128320 while base has 128256).
    On load, the leading ``base_vocab_size`` rows of ``model.embed_tokens.weight``
    and (when present as an independent parameter) ``lm_head.weight`` are kept,
    so all merge methods can combine these layers without shape mismatches.

    The base model itself is loaded unchanged. Models whose vocab already matches
    the base are also loaded unchanged. The base vocab size is read once from the
    base model's ``AutoConfig`` (cheap; no full model load).

    For Llama-3.2-3B and other models with ``tie_word_embeddings=True``,
    ``model.embed_tokens.weight`` and ``lm_head.weight`` reference the same
    underlying tensor, so a single truncation handles both. Lazy loading is not
    supported and is automatically disabled when truncation is enabled.
    """
    ...
    def __init__(
        ...
        *,
        ...
        model_kwargs: Optional[DictConfig] = None,
        enable_lazy_loading: bool = False,
        truncate_vocab_to_base: bool = False,
        base_model_name: str = "_pretrained_",
        **kwargs,
    ):
        super().__init__(
            models=models,
        ...
        self._cached_base_vocab_size: Optional[int] = None

    def _get_base_vocab_size(self) -> int:
        if self._cached_base_vocab_size is None:
            from transformers import AutoConfig

            base_path = self.get_model_path(self.base_model_name)
            cfg = AutoConfig.from_pretrained(base_path)
            self._cached_base_vocab_size = int(cfg.vocab_size)
            log.info(
                "TruncatedVocabCausalLMPool: base (%s) vocab_size = %d",
                base_path, self._cached_base_vocab_size,
            )
        return self._cached_base_vocab_size
    ...
    def load_model(
        ...
        **kwargs,
        ...
        if self.truncate_vocab_to_base and self.enable_lazy_loading:
            log.warning(
                "TruncatedVocabCausalLMPool does not support lazy loading when "
                "truncate_vocab_to_base=True. Falling back to normal loading."
            )
            self.enable_lazy_loading = False

        model = super().load_model(model_name_or_config, *args, **kwargs)

        if not self.truncate_vocab_to_base:
            return model
        # Skip the base model itself.
        if (
            isinstance(model_name_or_config, str)
            and model_name_or_config == self.base_model_name
        ):
            return model
        if not isinstance(model, PreTrainedModel):
            return model

        target_vocab = self._get_base_vocab_size()
        self._truncate_model_vocab(model, target_vocab, model_name_or_config)
        return model

    @staticmethod
    def _truncate_model_vocab(
        model: PreTrainedModel,
        target_vocab: int,
        model_name: str | DictConfig,
    ) -> None:
        """Truncate ``embed_tokens`` and ``lm_head`` to ``target_vocab`` rows in-place."""
        ...
        with _torch.no_grad():
            ...
            new_embed.weight.data.copy_(embed.weight.data[:target_vocab])

            tied = (
                hasattr(model, "lm_head")
                and isinstance(model.lm_head, nn.Linear)
                and model.lm_head.weight.data_ptr() == embed.weight.data_ptr()
            )
            ...
            if hasattr(model, "lm_head") and isinstance(model.lm_head, nn.Linear):
                ...
                if tied:
                    ...
                    new_lm.weight = new_embed.weight  # re-tie
                    if lm.bias is not None:
                        new_lm.bias.data.copy_(lm.bias.data[:target_vocab])
                    model.lm_head = new_lm
                elif lm.weight.shape[0] > target_vocab:
                    new_lm = nn.Linear(
                        lm.in_features,
                        target_vocab,
                        bias=lm.bias is not None,
                    ...
                    new_lm.weight.data.copy_(lm.weight.data[:target_vocab])
                    if lm.bias is not None:
                        new_lm.bias.data.copy_(lm.bias.data[:target_vocab])
                    model.lm_head = new_lm
            ...
            log.info(
                "TruncatedVocabCausalLMPool: truncated %s embed_tokens %d -> %d (tied=%s)",
                model_name, current, target_vocab, tied,
            )
