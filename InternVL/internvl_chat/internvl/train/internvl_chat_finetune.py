...
def build_datasets(
    ...
    if data_args.use_packed_ds:
        ...
        # 对于非 use_packed_ds 与非 use_data_resampling 的情况
        concat_dataset = ConcatDataset(datasets)
        indices = list(range(len(concat_dataset)))
        random.shuffle(indices)
        train_dataset = Subset(concat_dataset, indices)
        # 将 ConcatDataset 的 datasets 属性赋值到 Subset 上，供 Trainer 补丁使用
        train_dataset.datasets = concat_dataset.datasets
